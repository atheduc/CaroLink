...
function Post() {
  ...
  useEffect(() => {
    // Fetch post details
    axios
      .get(`${process.env.REACT_APP_API_URL}/posts/byId/${id}`)  // Use environment variable for API URL
      .then((response) => {
        setPostObject(response.data);
      })
      .catch((error) => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}/comments/${id}`)  // Use environment variable for API URL
      .then((response) => {
        setComments(response.data);
      })
      .catch((error) => {
  ...
  const addComment = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_URL}/comments`,  // Use environment variable for API URL
        {
          commentBody: newComment,
          PostId: id,
        },
  ...
  const deleteComment = (commentId) => {
    ...
    axios
      .delete(`${process.env.REACT_APP_API_URL}/comments/${commentId}`, {  // Use environment variable for API URL
        headers: { accessToken },
      })
      .then(() => {
        setComments(comments.filter((comment) => comment.id !== commentId));
          ...
          <div className="footer">
            ...
            {authState.username === postObject.username && (
              <button
                ...
                onClick={() => {
                  axios
                    .delete(`${process.env.REACT_APP_API_URL}/posts/${id}`, {  // Use environment variable for API URL
                      headers: { accessToken: localStorage.getItem("accessToken") },
                    })
                    .then(() => navigate("/"));
                }}
